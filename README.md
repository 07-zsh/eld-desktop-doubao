...
- **桌面首页**：大时钟（低频刷新、秒点动画）+ 日期 + 预置天气，六宫格大按钮（电话/家人/照片/SOS + 桌面第三行 2 个子女勾选的第三方应用），仅一屏无滑动。
...
- **桌面应用（功能3）**：子女在设置页从白名单（抖音/微信/快手）勾选最多 2 个应用，置于桌面第三行；点一下直接启动；应用被卸载后回到桌面自动隐藏（每次回桌面 onResume 重新校验安装状态）；采用 `<queries>` 点名可见，不使用 `QUERY_ALL_PACKAGES`。
- **数据层**：Room（联系人/紧急号码/桌面应用，支持子女增删改；应用表经 v1→v2 迁移）+ DataStore（已初始化/SOS 冷却）+ 应用私有目录（相册）；无服务器、无账号、无后台服务、无定位权限。
...
> 说明：本机已配置 JDK 17 + Android SDK + Gradle 8.7，单测已通过（30 个用例全绿，含功能1 新增 11 个、功能3 应用 5 个）；`assembleDebug` 生成 APK 可在此环境执行，并已在 `elder_test` 模拟器上跑通功能1（新增/编辑/删除）与功能3（六宫格/添加应用/启动/卸载隐藏/数据库 v1→v2 迁移）全流程。

## 首次使用

1. 安装 APK，点桌面图标「老年桌面」进入子女设置。
2. 授予电话/短信权限，自动导入 `assets/preset/` 预置数据。
3. （可选）在设置页点「管理家人」增删改联系人（姓名/手机号/头像/紧急标志）；点「管理应用」从白名单（抖音/微信/快手）勾选最多 2 个应用到桌面第三行。
