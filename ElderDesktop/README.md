...
- **子女设置模式**：唯一允许出现系统权限弹窗的阶段，一次性授予 `CALL_PHONE`/`SEND_SMS` 并从 `assets/preset/*` 预置联系人、紧急号码、相册。
- **数据层**：Room（联系人/紧急号码）+ DataStore（已初始化/SOS 冷却）+ 应用私有目录（相册）；无服务器、无账号、无后台服务、无定位权限。
...
./gradlew :app:testDebugUnitTest     # 运行纯 JVM 单测（SOS 状态机 + JSON 解析）
```

> 说明：交付环境未安装 Android SDK，故未在此机构建 APK；上述命令需在装有 Android SDK 的机器执行。
> 单测中的 `SosStateMachineTest`、`PresetParserTest` 不依赖 Android Framework，可纯 JVM 运行。

## 首次使用

1. 安装 APK，点桌面图标「老年桌面」进入子女设置。
2. 授予电话/短信权限，自动导入 `assets/preset/` 预置数据。
3. 按提示在系统设置中把「老年桌面」设为默认桌面。
4. 之后长辈开机即入大字桌面，全程零弹窗、零设置。

## 预置数据位置

子女可直接编辑后随 APK 发布：

- `app/src/main/assets/preset/contacts.json`
- `app/src/main/assets/preset/emergency.json`
- `app/src/main/assets/preset/weather.json`
- `app/src/main/assets/preset/photos/`（相册图片）
